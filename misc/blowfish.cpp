...
void Cblowfish::Submit_Key(data_ref key)
{
	int length = key.size();
	...
	*/
	...
	for (int index = 0; index < ROUNDS + 2; index++) {
		...
		data = (data << CHAR_BIT) | key[j++ % length];
		data = (data << CHAR_BIT) | key[j++ % length];
		data = (data << CHAR_BIT) | key[j++ % length];
		data = (data << CHAR_BIT) | key[j++ % length];
...
int Cblowfish::Encrypt(const void* plaintext, void* cyphertext, int length)
{
...
int Cblowfish::Decrypt(const void* cyphertext, void* plaintext, int length)
{
...
void Cblowfish::Process_Block(void const* plaintext, void* cyphertext, unsigned int const* ptable)
{
	/*
	...
	*/
	for (int index = 0; index < ROUNDS / 2; index++) {
		left.Long ^= *ptable++;
		right.Long ^=
			(((bf_S[0][left.Char.C0] + bf_S[1][left.Char.C1]) ^ bf_S[2][left.Char.C2]) + bf_S[3][left.Char.C3]);
		right.Long ^= *ptable++;
		left.Long ^=
...
void Cblowfish::Sub_Key_Encrypt(unsigned int& left, unsigned int& right)
{
	Int l;
	l.Long = left;
...
unsigned int const Cblowfish::P_Init[Cblowfish::ROUNDS + 2] =
{
	0x243F6A88U,
	0x85A308D3U,
	0x13198A2EU,
...
unsigned int const Cblowfish::S_Init[4][UCHAR_MAX + 1] =
{
		{
