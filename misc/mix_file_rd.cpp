...
int Cmix_file_rd::post_open()
{
	...
	if (m_ft_support)
	{
		...
		if (m_index.size())
		{
			// Extension-driven fallback. Also runs for non-encrypted MIXes to
			// fill in entries that the byte-probe couldn't classify (e.g. RA1
			// .wsa whose is_valid() rejects MIX-internal records, .icn/.fnt/
			// .urb that have no probe, or streamed MIXes where can_probe was
			// false and m_index_ft stayed empty). Only fills ft_unknown
			// entries — real probe matches stay.
			...
			for (int i = 0; i < get_c_files(); i++)
			{
				...
				// .urb / .icn share enough header shape with SHP/TMP that the
				// content probe returns the wrong variant. Force these by
				// extension even when the probe classified them.
